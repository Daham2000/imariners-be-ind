...
export default class CategoryService {
    ...
    async addCategory(category: Category): Promise<any> {
        ...
        const subCategoriesResult = await this.categoryDAO.getSubCategories(category.categoryId);
        let subCategories = [];
    ...
    async getCategories(dataModel: DataModel) {
        ...
        for (let index = 0; index < categories.length; index++) {
            ...
            const subCategoriesResult = await this.categoryDAO.getSubCategories(categories[index].c_id);
            let subCategories = [];
            for (let i = 0; i < subCategoriesResult.length; i++) {
                let content_links_sub = [];
                let subCategoryLinks = JSON.parse(subCategoriesResult[i].content_links);
